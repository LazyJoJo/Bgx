...
|------|--------|-----|-----|
|id |BIGSERIAL |PK |主键，自增 |
|stock_id |BIGINT |NOT NULL, FK |ID，关联stock_basic |
...
--行数据表索引
CREATE INDEX idx_stock_quote_stock_time ON stock_quote(stock_id, quote_time);
CREATE INDEX idx_fund_quote_fund_time ON fund_quote(fund_id, quote_time);
...
- `(stock_id, quote_time)`组唯一 (同一股票同一时间点数据唯一)
- `(fund_id, quote_time)`组唯一 (同一基金同一时间点数据唯一)
...
```bash
# 使用psql命令行工具
psql -U postgres -d stock_fund_db -f final_init_tables.sql
...
- **主要改进**: 
  -了PostgreSQL关键字冲突
  - 使用语义化字段名`quote_time`替代`timestamp`
  - 保持了所有注释的完整性
  -确保了全链路一致性