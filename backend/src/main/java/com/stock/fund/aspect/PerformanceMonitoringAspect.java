...
public class PerformanceMonitoringAspect {
    ...
    public Object monitorDataCollectionPerformance(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (stopWatch.getTotalTimeMillis() > 5000) { // 5秒阈值
                logger.warn("Data collection method execution time too long: {}, elapsed: {}ms", methodKey,
                        stopWatch.getTotalTimeMillis());
            }

            // 定输出统计信息
            if (stats.getCallCount() % 100 == 0) {
                logger.info("Method performance stats [{}]: callCount={}, avgTime={}ms, maxTime={}ms", methodKey,
                        stats.getCallCount(), stats.getAverageTime(), stats.getMaxTime());
            }

            return result;
        } catch (Exception e) {
            stopWatch.stop();
            logger.error("Data collection method execution exception: {}, elapsed: {}ms, exception: {}", methodKey,
    ...
    public Object monitorSchedulerPerformance(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        logger.info("Starting scheduled task: {}", methodKey);
        ...
        try {
            ...
            logger.info("Scheduled task completed: {}, elapsed: {}ms", methodKey, stopWatch.getTotalTimeMillis());

            // 记录性能警告
            if (stopWatch.getTotalTimeMillis() > 30000) { // 30秒阈值
                logger.warn("Scheduled task execution time too long: {}, elapsed: {}ms", methodKey,
                        stopWatch.getTotalTimeMillis());
            }
            ...
            stopWatch.stop();
            logger.error("Scheduled task execution exception: {}, elapsed: {}ms, exception: {}", methodKey,
                    stopWatch.getTotalTimeMillis(), e.getMessage());
    ...
    public Object monitorDatabasePerformance(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (stopWatch.getTotalTimeMillis() > 1000) {
                logger.warn("Database operation time too long: {}, elapsed: {}ms", methodKey,
                        stopWatch.getTotalTimeMillis());
            }
            ...
            stopWatch.stop();
            logger.error("Database operation exception: {}, elapsed: {}ms, exception: {}", methodKey,
                    stopWatch.getTotalTimeMillis(), e.getMessage());
            throw e;
        }
    }
    ...
    public Object monitorHttpPerformance(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (stopWatch.getTotalTimeMillis() > 5000) {
                logger.warn("HTTP request time too long: {}, elapsed: {}ms", methodKey, stopWatch.getTotalTimeMillis());
            }

            return result;
        } catch (Exception e) {
            stopWatch.stop();
            logger.error("HTTP request exception: {}, elapsed: {}ms, exception: {}", methodKey,
                    stopWatch.getTotalTimeMillis(), e.getMessage());
            throw e;
        }
    }
    ...
    private static class MethodStats {
        ...
        public void update(long executionTime) {
            ...
            while (executionTime > currentMax) {
                if (java.util.concurrent.atomic.AtomicLongFieldUpdater.newUpdater(MethodStats.class, "maxTime")
                        .compareAndSet(this, currentMax, executionTime)) {
                    break;
                }
                currentMax = maxTime;
            }
