...
public class ThreadPoolUtils {
    ...
    public static ExecutorService getGlobalExecutor() {
        if (globalExecutor == null) {
            synchronized (ThreadPoolUtils.class) {
                if (globalExecutor == null) {
                    globalExecutor = createExecutorService(DEFAULT_CORE_POOL_SIZE, DEFAULT_MAX_POOL_SIZE,
                            DEFAULT_QUEUE_SIZE, DEFAULT_THREAD_FACTORY);
                }
            }
        }
    ...
    public static ExecutorService createExecutorService(int corePoolSize, int maxPoolSize, int queueSize,
        ...
        return new java.util.concurrent.ThreadPoolExecutor(corePoolSize, maxPoolSize, 60L,
                java.util.concurrent.TimeUnit.SECONDS, new java.util.concurrent.LinkedBlockingQueue<>(queueSize),
        ...
        if (executor != null && !executor.isShutdown()) {
            try {
                ...
                if (!executor.awaitTermination(60, java.util.concurrent.TimeUnit.SECONDS)) {
                    ...
                    if (!executor.awaitTermination(60, java.util.concurrent.TimeUnit.SECONDS)) {
                        log.warn("Thread pool failed to shutdown gracefully");
                    }
                }
