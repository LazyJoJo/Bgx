...
/**
 * RiskAlertController API 测试 使用 MockMvc 进行 Controller 层测试 通过 @MockBean
 * 禁用调度器避免 @Scheduled 问题
 */
@SpringBootTest
...
class RiskAlertControllerTest {
        ...
        void getUserRiskAlerts_shouldReturnMergedAlerts() throws Exception {
                // given
                RiskAlertMergeDTO stockAlert = new RiskAlertMergeDTO(1L, "000001", "STOCK", "平安银行", LocalDate.now(),
                                "ACTIVE", 2, new BigDecimal("5.50"), new BigDecimal("1.00"), new BigDecimal("3.20"),
                                new BigDecimal("12.50"), new BigDecimal("11.80"), false, LocalDateTime.now(),
                                List.of());
                RiskAlertMergeDTO fundAlert = new RiskAlertMergeDTO(2L, "000011", "FUND", "测试基金", LocalDate.now(),
                                "ACTIVE", 1, new BigDecimal("3.50"), new BigDecimal("1.50"), new BigDecimal("3.50"),
                                new BigDecimal("1.5500"), new BigDecimal("1.4800"), false, LocalDateTime.now(),
                                List.of());
                when(riskAlertAppService.getMergedRiskAlerts(eq(1L), any(), anyInt()))
                                .thenReturn(Arrays.asList(stockAlert, fundAlert));

                // when & then
                mockMvc.perform(get("/api/risk-alerts/user/1").param("limit", "20")).andExpect(status().isOk())
        ...
        void getTodayRiskAlerts_shouldReturnGroupedAlerts() throws Exception {
                // given
                RiskAlertSummaryDTO.RiskAlertItemDTO item1 = RiskAlertSummaryDTO.RiskAlertItemDTO.builder().id(1L)
                                .symbol("000001").symbolType("STOCK").symbolName("平安银行").timePoint("14:30")
                                .hasRisk(true).changePercent(new BigDecimal("5.50"))
                                .currentPrice(new BigDecimal("12.50")).yesterdayClose(new BigDecimal("11.80"))
                                .isRead(false).triggeredAt("2026-04-11 14:30:00").build();

                RiskAlertSummaryDTO.RiskAlertItemDTO item2 = RiskAlertSummaryDTO.RiskAlertItemDTO.builder().id(2L)
                                .symbol("000011").symbolType("FUND").symbolName("测试基金").timePoint("14:30").hasRisk(true)
                                .changePercent(new BigDecimal("3.50")).currentPrice(new BigDecimal("1.5500"))
                                .yesterdayClose(new BigDecimal("1.4800")).isRead(false)
                                .triggeredAt("2026-04-11 14:30:00").build();

                RiskAlertSummaryDTO summary = RiskAlertSummaryDTO.builder().alertDate(LocalDate.now()).totalCount(2)
                                .unreadCount(2).items(Arrays.asList(item1, item2)).build();
        ...
        void getUnreadCount_shouldReturnCount() throws Exception {
                ...
                // when & then
                mockMvc.perform(get("/api/risk-alerts/user/1/unread-count")).andExpect(status().isOk())
        ...
        void markAsRead_shouldMarkAlertAsRead() throws Exception {
                ...
                // when & then
                mockMvc.perform(patch("/api/risk-alerts/1/read")).andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true));
        ...
        void markAllAsRead_shouldMarkAllAlertsAsRead() throws Exception {
                ...
                // when & then
                mockMvc.perform(post("/api/risk-alerts/user/1/mark-read")).andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true));
        ...
        void getTodayRiskAlertCount_shouldReturnCount() throws Exception {
                ...
                // when & then
                mockMvc.perform(get("/api/risk-alerts/user/1/today-count")).andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true))
        ...
        void checkRiskAlerts_shouldTriggerCheck() throws Exception {
                ...
                mockMvc.perform(post("/api/risk-alerts/check")).andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true));
        ...
        void deleteRiskAlert_shouldDeleteAlert() throws Exception {
                ...
                // when & then
                mockMvc.perform(delete("/api/risk-alerts/1")).andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true));
        ...
        void getUserRiskAlerts_withCursor_shouldReturnPaginatedAlerts() throws Exception {
                // given
                RiskAlertMergeDTO alert = new RiskAlertMergeDTO(10L, "000002", "STOCK", "招商银行", LocalDate.now(),
                                "ACTIVE", 1, new BigDecimal("2.30"), new BigDecimal("1.00"), new BigDecimal("2.30"),
                                new BigDecimal("35.80"), new BigDecimal("35.00"), false, LocalDateTime.now(),
                                List.of());
                when(riskAlertAppService.getMergedRiskAlerts(eq(1L), eq(1234567890L), anyInt()))
                                .thenReturn(List.of(alert));

                // when & then
                mockMvc.perform(get("/api/risk-alerts/user/1").param("cursor", "1234567890").param("limit", "10"))
